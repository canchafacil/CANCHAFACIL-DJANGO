document.addEventListener('DOMContentLoaded', function () {
    let calendar; // se inicializa solo una vez, se reutiliza después

    const modalCalendario = document.getElementById('modalCalendarioReservas');
    if (!modalCalendario) return; // por si esta página no tiene el calendario
    ...
    function cargarCalendario() {
        ...
        fetch("/administracion/calendario/eventos/")
            .then(res => res.json())
            .then(data => {
                const eventos = Object.entries(data.fechas).map(([fecha, info]) => ({
                    start: fecha,
                    display: 'background',
                    color: (info.pasada && !info.activa) ? '#5a5a5a' : '#3b82f6',
                }));
    ...
    function cargarHorasDelDia(fecha) {
        fetch(`/administracion/calendario/dia/${fecha}/`)
            .then(res => res.json())
            .then(data => {
    ...
    function verDetalleReserva(id) {
        fetch(`/administracion/calendario/reserva/${id}/`)
            .then(res => res.json())
            .then(r => {
